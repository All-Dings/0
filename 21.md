...
She pretends to me to not have understood my [Argumentation](600176.md), which of course cannot be [The-Truth](600140.md), because she is not bad at all in [Mathematics](13000000.md) and of course knows the following [Formula](13000025.md):

$$ Average(x_1,x_2 ..\:x_n) = \frac{\sum\limits_{i=1}^{n}{x_i}}{n} = \frac{x_1 + x_2 + .. + x_n}{n} $$

After a Walk in [Harmony](60100.md) with [Soey](142000001.md), the next Day I meet [Sarah](151000002.md) together with her [Father](40000007.md), who seem to me not so experienced in [Theory](7000001.md) and therefore I try to explain also them as simple as possible, why [Men](40080022.md) *really* do *not* have more [Fun](60170.md) than [Women](40000010.md).

So let's assume we have a [Micro-Universe](10000027.md) with only two Men **M1/M2** and two Women **W1/W2**, where **M1** has two Contacts with each of **W1/W2**, whereas **M2** has Zero-Contacts, then we can calculate the [Average-Ratio](12000060.md) **Women_per_Men** as Follows:

$$ \mathrm{Women\_for\_M1} = 2 $$
$$ \mathrm{Women\_for\_M2} = 0 $$
$$ \mathrm{Number\_of\_Contacts} = 2 $$

$$ \mathrm{Women\_per\_Men} = \frac{\mathrm{Women\_for\_M1} + \mathrm{Women\_for\_M2}}{\mathrm{Number\_of\_Contacts}} $$
$$ = \frac{2 + 0}{2} = \frac{2}{2} = 1 $$

So on Average each Man has one female Contact - But now let's see, if Women *really* have an Advantage here:

$$ \mathrm{Men\_for\_W1} = 1 $$
$$ \mathrm{Men\_for\_W2} = 1 $$
$$ \mathrm{Number\_of\_Contacts} = 2 $$

$$ \mathrm{Men\_per\_Women} = \frac{\mathrm{Men\_for\_W1} + \mathrm{Men\_for\_W2}}{\mathrm{Number\_of\_Contacts}} $$
$$ = \frac{1 + 1}{2} = \frac{2}{2} = 1 $$

Tada, $\mathrm{Women\_per\_Men} = \mathrm{Men\_per\_Women}$ and who was right again? But I have to admit, that though in [Theory](7000001.md) this looks like mathematic [Correctness](600077.md), nevertheless in [Reality](40000.md) the unbalanced Distribution could lead to [Loneliness](14000003.md) for **M2** and (maybe too much) hard [Work](60036.md) for **M1**:
