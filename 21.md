...
For Example, something like "Moral is a [Measure](10000021.md) for [Decisions](60095.md) to achieve a [Goal](60058.md)". Here a Proposal for a [Pyhthon](9010003.md)-Program:

```python
def Profit(Person, Descision)
    World_1 = World(Current)
    Value_1 = World_1.Get_Value_For(Person)
    World_2 = World_1.Do_Actions(Decision)
...
But this Definition might be wrong, so as a Decision for the Time being let's formualte it trivial, but at least correct:
